...
Bukkit Essentials is a plugin for a Bukkit server that adds a small but useful set of miscellaneous command-line tools. The current version is confirmed to work with Spigot-CraftBukkit version 1.8.8, and will maybe work on later versions without modification. A full list of commands that this plugin adds is included below.
